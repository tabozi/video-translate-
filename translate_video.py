...
def transcribe_audio(audio_path, device, model_size="tiny"):
    """Transcribe audio to text using Whisper."""
    logging.info(f"Starting audio transcription with Whisper (model: {model_size})")
    try:
        if device == "cuda":
...
def combine_video_audio(video_path, audio_path, output_filename):
    ...
    try:
        ...
        # FFmpeg command for NVENC encoding with progress
        cmd = [
            'ffmpeg', '-y',
            '-i', video_path,
            '-i', temp_audio,
        ...
        if process.wait() == 0:
            ...
            logging.warning("NVENC failed, falling back to CPU...")
...
def main():
    ...
    text = transcribe_audio(audio_path, device, args.model)
    if text is None:
        return
